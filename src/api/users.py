...
from .middlewares import admin_required, agency_required, owner_required
...
def get_all_users():
    ...
    pagination = User.get_users_list().paginate(
...
def get_all_companies():
    ...
    pagination = User.get_companies_list().paginate(
        page=page, per_page=per_page, error_out=False
    )
...
def get_all_lessors():
    ...
    pagination = User.get_lessors_list().paginate(
        page=page, per_page=per_page, error_out=False
    )
...
@api_bp.post("/owners/tenant-register/")
@login_required
@owner_required
@jsonify_response
...
@api_bp.post("/companies/tenant-register/")
@login_required
@agency_required
@jsonify_response
...
def login():
    ...
    user = User.find_by_email_and_phone(email_or_phone)
