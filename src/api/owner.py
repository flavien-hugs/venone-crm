from flask import g, abort, jsonify, request, flash, url_for
...
def get_all_houseowners():
    ...
    pagination = VNHouseOwner.get_houseowners_list()\
...
def delete_houseowner(owner_uuid):
    ...
    if owner is not None:
        owner.disable()
        return jsonify({'success': True, 'message': f"Le compte du bailleur {owner} a été supprimé avec succès."})
    return jsonify({'success': False, 'message': f"L'élément avec l'id {owner_uuid} n'a pas été trouvé."})
...
def get_houseowner_houses(owner_uuid):
    ...
    pagination = houseowner.houses\
        .paginate(page=page, per_page=per_page, error_out=False)

    houses = pagination.items
    prev = None
    if pagination.has_prev:
        prev = url_for('api.get_houseowner_houses', uuid=owner_uuid, page=page-1, _external=True)
    next = None
    if pagination.has_next:
        next = url_for('api.get_houseowner_houses', uuid=owner_uuid, page=page+1, _external=True)
...
def get_houseowner_tenants(owner_uuid):
    ...
    pagination = houseowner.tenants\
        .paginate(page=page, per_page=per_page, error_out=False)

    tenants = pagination.items
    prev = None
    if pagination.has_prev:
        prev = url_for('api.get_houseowner_tenants', uuid=owner_uuid, page=page-1, _external=True)
    next = None
    if pagination.has_next:
        next = url_for('api.get_houseowner_tenants', uuid=owner_uuid, page=page+1, _external=True)
