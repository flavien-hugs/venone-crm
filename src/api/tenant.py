...
def get_all_tenants():
    ...
    pagination = VNTenant.get_tenants_list()\
...
def delete_tenant(tenant_uuid):
    ...
    if tenant is not None:
        tenant.delete()
        return jsonify({'success': True, 'message': f"Le locataire {tenant} a été supprimé avec succès."})
    return jsonify({'success': False, 'message': f"L'élément avec l'id {tenant_uuid} n'a pas été trouvé."})
