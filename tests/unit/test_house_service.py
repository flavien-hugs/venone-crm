...
def house_service(mock_house_repository, mock_owner_repository, mock_tenant_repository):
    ...
    return HouseService(mock_house_repository, mock_owner_repository, mock_tenant_repository)

def test_update_lease_end_date_if_expired(app_context, house_service, mock_house_repository):
    ...
    with patch('src.core.services.house_service.db') as mock_db:
        ...
        assert expired_house.vn_house_lease_end_date == date.today() + timedelta(days=29)
...
def test_get_current_tenant_id_no_tenant(app_context, house_service, mock_tenant_repository):
