...
def test_get_total_payments_received_for_owner(mock_db, user_service, mock_user_repository, sample_user_entity):
    ...
    mock_query = Mock()
    mock_db.session.query.return_value.filter.return_value.join.return_value.join.return_value.join.return_value.filter.return_value.scalar.return_value = 150000.0
...
def test_calculate_amount_apply_percent_user_not_found(user_service, mock_user_repository):
...
def test_calculate_amount_apply_percent_for_owner(mock_house, user_service, mock_user_repository, sample_user_entity):
...
def test_get_dashboard_stats_for_user(mock_transfer, mock_owner, mock_tenant, mock_house, mock_payment, user_service, mock_user_repository, sample_user_entity):
    ...
    mock_house.query.filter_by.return_value.count.side_effect = [2, 1, 1] # houses_count, houses_close_count, houses_open_count
